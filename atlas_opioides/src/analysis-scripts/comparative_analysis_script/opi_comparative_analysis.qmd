...
df_piramide_ <- df_piramide_ %>% group_by(ccaa_cd) %>% mutate(total_pobla = sum(sum_n_personas,na.rm=TRUE))
...
data_new_ind_ <- data_new_ind_ %>% filter(str_starts(atc_farmaco_dispensado,'N02AX02|N02AJ13|N02AJ14')) %>% 
group_by(año_cd,ccaa_cd,zbs_residencia_cd,total) %>% summarise(percent = (sum(sum_n_envases,na.rm=TRUE))) 
data_new_ind_$percent <- round((100*(data_new_ind_$percent/data_new_ind_$total)),2)
...
plot <- ggplot(data=data_new_ind_,aes(x=factor(año_cd), y=percent, group = año_cd)) +
    ...
    geom_point(size=0.7,alpha = 0.3) +
    geom_line(aes(x=factor(año_cd),y=p50,group =ccaa_st,color='P50 nacional')) +
    labs(title= 'Porcentaje de tramadol (sólo o en combinación) (N02AX02, N02AJ13, N02AJ14) respecto al total de opioides (N02A)',
         x = '',
         y='Porcentaje (%)')+facet_wrap(~ccaa_st,ncol = 4) +
    theme_minimal() +
    # scale_color_manual(name = "",values = c(
...
data_new_ind_ <- data_new_ind %>% filter(atc_farmaco_dispensado %in% c('N02AJ13','N02AX02','N02AJ14'))

data_new_ind_ <- data_new_ind_ %>% group_by(año_cd,ccaa_cd,zbs_residencia_cd) %>% mutate(total = sum(sum_n_envases,na.rm=TRUE))

data_new_ind_ <- data_new_ind_ %>% filter(atc_farmaco_dispensado %in% c('N02AJ13','N02AJ14')) %>% 
group_by(año_cd,ccaa_cd,zbs_residencia_cd,total) %>% summarise(percent = (sum(sum_n_envases,na.rm=TRUE))) 
data_new_ind_$percent <- round((100*(data_new_ind_$percent/data_new_ind_$total)),2)
...
plot <- ggplot(data=data_new_ind_,aes(x=factor(año_cd), y=percent, group = año_cd)) +
    ...
    geom_point(size=0.7,alpha = 0.3) +
    geom_line(aes(x=factor(año_cd),y=p50,group =ccaa_st,color='P50 nacional')) +
    labs(title= 'Porcentaje de tramadol en combinación (N02AJ13, N02AJ14) respecto al total de tramadol dispensado (N02AJ13 + N02AJ14 + N02AX02)',
         x = '',
         y='Porcentaje (%)')+facet_wrap(~ccaa_st,ncol = 4) +
    theme_minimal() +
    # scale_color_manual(name = "",values = c(
...
plot <- tm_shape(mapa_)  + tm_fill('percent', title = "Deciles",
                        ...
                        tm_borders("gray50", alpha = 0.8, lwd = 0.0) +
                                tm_facets(by = "ccaa_st", free.scales = TRUE,showNA = FALSE) +
                        # tm_borders('#6e0c05', lwd = 1.2) +
              tm_layout(title='Porcentaje de tramadol (sólo o en combinación) (N02AX02, N02AJ13, N02AJ14) respecto al total de opioides (N02A)',title.position =  c("LEFT", "TOP"),
title.size = 0.95,legend.outside = TRUE,legend.show = TRUE,
              legend.outside.position = c('bottom'),outer.margins = c(-0.175,0,0,0),
              panel.label.bg.color = "white")
...
plot <- tm_shape(mapa_)  + tm_fill('percent', title = "Deciles",
                        ...
                        tm_borders("gray50", alpha = 0.8, lwd = 0.0) +
                                tm_facets(by = "ccaa_st", free.scales = TRUE,showNA = FALSE) +
                        # tm_borders('#6e0c05', lwd = 1.2) +
              tm_layout(title='Porcentaje de tramadol en combinación (N02AJ13, N02AJ14) respecto al total de tramadol dispensado (N02AJ13 + N02AJ14 + N02AX02)',title.position =  c("LEFT", "TOP"),
title.size = 0.95,legend.outside = TRUE,legend.show = TRUE,
              legend.outside.position = c('bottom'),outer.margins = c(-0.175,0,0,0),
              panel.label.bg.color = "white")
...
- Indicadores de utilización de analgésicos opioides:
  ...
  - Porcentaje de tramadol (sólo o en combinación) (*N02AX02*, *N02AJ13*, *N02AJ14*) respecto al total de opioides (*N02A*)
  ...
  - Porcentaje de tramadol en combinación (*N02AJ13*, *N02AJ14*) respecto al total de tramadol dispensado (*N02AJ13* + *N02AJ14* + *N02AX02*)
