...
df <- df %>% rename(std_rate = te, cum_var = total, year = año)
write.table(df, file = paste0(outputs_path,"antibioticos_indicators_rates.csv"),
            row.names = FALSE, sep = '|')
...
plot_te_per_year <- function(var){
 ...
 #for(file in names_files){
  for(ind in df_list_indicators$indicators){
    ...
    plot <- ggplot(data=data_filter, aes(x=year, y=std_rate, group = year)) +
      ...
      theme_minimal_hgrid() + 
      geom_hline(data=data_filter, mapping=aes(yintercept=median(std_rate,na.rm = TRUE)), color="red") +
      geom_text(x=2023, y=max(data_filter$std_rate,na.rm=TRUE), label="| P50 (2013-2022)", color = 'red', size=4,hjust=1) + 
      geom_text(x=2023, y=median(data_filter$std_rate,na.rm = TRUE), label=paste0('                ',round(median(data_filter$std_rate,na.rm = TRUE),2)), color = 'red', size=4) +
      geom_text(x=2022.5, y=max(data_filter$std_rate,na.rm=TRUE), label="| P50 año", color = 'black',hjust=1) + 
      coord_flip()
    p[[ind]] <- plot
  }
  return(p)
