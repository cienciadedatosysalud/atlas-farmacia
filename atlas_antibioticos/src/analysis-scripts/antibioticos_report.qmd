...
source('./auxiliar_scripts/farmacia_standardized_antibioticos3.R')
rm(list=ls())
log_info('Calculando tasas estandarizadas e indicadores estadísticos para ATC 4 caracteres')
source('./auxiliar_scripts/farmacia_standardized_antibioticos4.R')
rm(list=ls())
```
...
write.table(df, file = paste0(outputs_path,"antibioticos_estadisticosobs.csv"),
            row.names = FALSE, sep = '|')
...
write.table(df, file = paste0(outputs_path,"antibioticos_dhd_pvp_population_values.csv"),
            row.names = FALSE, sep = '|')
...
write.table(df_, file = paste0(outputs_path,"antibioticos_dhd_pvp_resumen_anual.csv"),
            row.names = FALSE,  sep = '|')
...
El nomenclator usado como referencia para todos los años es el nomenclator del Ministerio de Sanidad (septiembre 2025) para todas las CCAA en el que se han modificado los DDD de los fármacos combinados para especificar el DDD del fármaco de interés (antibióticos). 
...
## Porcentaje de población con dispensaciones de antibióticos por edad y sexo en el último año disponible 
::: {.justify}
...
::: {.justify}
Se muestra el consumo (DHD) para el grupo mayor J01 por ZBS en el último año disponible, ordenados de menor a mayor y coloreados por quintiles.
:::
::: {.callout-tip}
## Nota informativa
...
consumo_zbs_total_ <- read.csv('../../outputs/antibioticos_dhd_pvp_population_values.csv', sep = '|')
consumo_zbs_total_ <- consumo_zbs_total_ %>% filter(year %in% max(year,na.rm=TRUE))
consumo_zbs_total_ <- consumo_zbs_total_ %>% filter(variable %in% 'dhd', indicator %in% 'J01')
...
plot <- ggplot(df_,
  ...
  scale_fill_manual(values = c("J01C"="#66c2a5", "J01D"="#fc8d62", "J01F"="#8da0cb", "J01MA"='#e78ac3', "J01X"='#a6d854')) +
...
data <- read.csv('../../outputs/antibioticos_dhd_pvp_population_values.csv', sep = "|",colClasses=c("codatzbs"="character"))
 ...
 #for(file in names_files){
  for(ind in df_list_indicators$indicators){
    ...
    plot <- ggplot(data=data_filter, aes(x=year, y=std_value, group = year)) +
      ...
      scale_x_continuous(limits = c(min(data_filter$year)-1, max(data_filter$year)+1), breaks = seq(min(data_filter$year), max(data_filter$year), by = 1)) +
      theme_minimal_hgrid() + 
      geom_hline(data=data_filter, mapping=aes(yintercept=median(std_value,na.rm = TRUE)), color="red") +
      geom_text(x=max(data_filter$year)+1, y=max(data_filter$std_value,na.rm=TRUE), label="| P50 (serie completa)", color = 'red', size=4,hjust=1) + 
      geom_text(x=max(data_filter$year)+1, y=median(data_filter$std_value,na.rm = TRUE), label=paste0('                ',round(median(data_filter$std_value,na.rm = TRUE),2)), color = 'red', size=4) +
      geom_text(x=max(data_filter$year)+0.5, y=max(data_filter$std_value,na.rm=TRUE), label="| P50 año", color = 'black',hjust=1) + 
      coord_flip()
    p[[ind]] <- plot
  }}
  return(p)
