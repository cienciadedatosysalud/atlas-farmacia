...
select
	...
	zbs_residencia_cd,
	ccaa_cd,
	atc_farmaco_dispensado_cd[1:3] as atc_farmaco_dispensado,
...
select a.* from (select atc_farmaco_dispensado as atc_farmaco_dispensado ,ccaa_cd, zbs_residencia_cd ,sexo_cd, grupo_edad_cd, tsi_cd , 
...
from partial_df where año = ",año," and sexo_cd in (1,2) group by atc_farmaco_dispensado ,ccaa_cd, zbs_residencia_cd ,sexo_cd, grupo_edad_cd, tsi_cd) a"
  ))
...
#   write.csv(indicators_rates, file = paste0(outputs_path,"antibioticos3_indicators_rates_",as.character(variable_indicator_name),".csv"), 
#             row.names = FALSE)
#   write.csv(estadisticosobs, file = paste0(outputs_path,"antibioticos3_estadisticosobs_",as.character(variable_indicator_name),".csv"), 
#             row.names = FALSE)
# }
...
for(variable_indicator in c('ddd_por_envase','pvp_nomenclator_nm')){
  ...
  write.csv(indicators_rates_total, file = paste0(outputs_path,"antibioticos3_indicators_rates_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
  write.csv(estadisticosobs_total, file = paste0(outputs_path,"antibioticos3_estadisticosobs_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
}
