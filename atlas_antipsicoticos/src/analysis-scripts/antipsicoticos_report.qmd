...
nomenclator_df <- read.csv(file = 'nomenclator_antipsicoticos.csv',sep='|',colClasses = c('codigo_nacional_farmaco_cd'='character'))
...
poblaciones_tarjeta_df <- dbGetQuery(conn = con, "
   SELECT * FROM 'datos_nivel_zona_basica_tarjeta.parquet' WHERE tsi_cd is NULL or tsi_cd NOT IN ('8')
	")
...
dbExecute(con, "CREATE OR REPLACE VIEW envase_dispensado_view AS SELECT envase_id::VARCHAR || '-' || year(fecha_dispensacion_dt)::VARCHAR as envase_id, fecha_dispensacion_dt, paciente_id, edad_nm, sexo_cd, tsi_cd, zbs_residencia_cd, ccaa_cd, a.codigo_nacional_farmaco_cd,n.atc_farmaco_dispensado_cd
, n.ddd_nomenclator_nm, n.ddd_por_envase, n.pvp_nomenclator_nm, n.precio_por_envase_nm,n.depot_bl  FROM (SELECT * from main.envase_dispensado 
WHERE tsi_cd is NULL or tsi_cd NOT IN ('8')) a
 LEFT JOIN main.nomenclator n
 ON a.codigo_nacional_farmaco_cd = n.codigo_nacional_farmaco_cd
 where n.codigo_nacional_farmaco_cd is not null AND NOT (n.depot_bl OR n.atc_farmaco_dispensado_cd = 'N05AN01')")

dbExecute(con, "CREATE OR REPLACE VIEW envase_dispensado_view_depot AS SELECT envase_id::VARCHAR || '-' || year(fecha_dispensacion_dt)::VARCHAR as envase_id, fecha_dispensacion_dt, paciente_id, edad_nm, sexo_cd, tsi_cd, zbs_residencia_cd, ccaa_cd, a.codigo_nacional_farmaco_cd,n.atc_farmaco_dispensado_cd
, n.ddd_nomenclator_nm, n.ddd_por_envase, n.pvp_nomenclator_nm, n.precio_por_envase_nm,n.depot_bl  FROM (SELECT * from main.envase_dispensado 
WHERE tsi_cd is NULL or tsi_cd NOT IN ('8')) a
 LEFT JOIN main.nomenclator n
 ON a.codigo_nacional_farmaco_cd = n.codigo_nacional_farmaco_cd
 where n.codigo_nacional_farmaco_cd is not null AND n.depot_bl")

dbExecute(con, "CREATE OR REPLACE VIEW envase_dispensado_view_litio AS SELECT envase_id::VARCHAR || '-' || year(fecha_dispensacion_dt)::VARCHAR as envase_id, fecha_dispensacion_dt, paciente_id, edad_nm, sexo_cd, tsi_cd, zbs_residencia_cd, ccaa_cd, a.codigo_nacional_farmaco_cd,n.atc_farmaco_dispensado_cd
, n.ddd_nomenclator_nm, n.ddd_por_envase, n.pvp_nomenclator_nm, n.precio_por_envase_nm,n.depot_bl  FROM (SELECT * from main.envase_dispensado 
WHERE tsi_cd is NULL or tsi_cd NOT IN ('8')) a
 LEFT JOIN main.nomenclator n
 ON a.codigo_nacional_farmaco_cd = n.codigo_nacional_farmaco_cd
 where n.codigo_nacional_farmaco_cd is not null AND n.atc_farmaco_dispensado_cd = 'N05AN01'")
...
total <- dbGetQuery(conn = con, '
   SELECT COUNT(*) as total FROM main.envase_dispensado_view
	')
list_ccaa <- dbGetQuery(conn = con, '
   SELECT DISTINCT ccaa_cd as ccaa_cd FROM main.envase_dispensado_view
	')
...
select
	...
	precio_por_envase_nm
from
	main.envase_dispensado_view_litio)
select
	a.*
from
	(
...
write.table(aggregated_output,'../../outputs/aggregated_outputs_litio.csv', sep = '|', row.names = FALSE)
rm(aggregated_output)
...
select
	...
	precio_por_envase_nm
from
	main.envase_dispensado_view_depot)
select
	a.*
from
	(
...
write.table(aggregated_output,'../../outputs/aggregated_outputs_depot.csv', sep = '|', row.names = FALSE)
rm(aggregated_output)

dbExecute(con,"DROP VIEW IF EXISTS envase_dispensado_view_depot")
dbExecute(con,"DROP VIEW IF EXISTS envase_dispensado_view_litio")
...
- 7 = "TSI 005-03 pensionistas tipo máximo".
...
df_list_indicators <- data.frame(indicators = c('N05AA','N05AB','N05AC','N05AD','N05AE','N05AF','N05AG','N05AH','N05AL','N05AX'),
...
list_atc_4 <- data.frame(codigo = c('N05AA','N05AB','N05AC','N05AD','N05AE','N05AF','N05AG','N05AH','N05AL','N05AX'),
...
plot_evo_temp_atc_nenv[[10]]$N05AX
```
...
tabla_envases_año$atc_farmaco_dispensado_short[tabla_envases_año$atc_farmaco_dispensado_short %nin% c('N05AA','N05AD','N05AE','N05AH','N05AL','N05AX')] <- 'Otros'
...
plot <- ggplot(df_,
  ...
  scale_fill_manual(values = c("N05AA"="#66c2a5", "N05AD"="#fc8d62", "N05AE"="#8da0cb", "N05AH"='#e78ac3', "N05AL"='#a6d854',
                               "N05AX"="#e5c494","Otros"="#8dd3c7")) +
...
evolucion_temporal_tsi$atc_farmaco_dispensado_short[evolucion_temporal_tsi$atc_farmaco_dispensado_short %nin% c('N05AA','N05AD','N05AE','N05AH','N05AL','N05AX')] <- 'Otros'
...
df_autores <- data.frame(ccaa_st = c('Aragón','Principado de Asturias','Islas Baleares','Cantabria','Cataluña','Canarias','Castilla y León',
                         ...
                         autores_st = c('Francisco Estupiñán Romero, Javier González Galindo, Santiago Royo Sierra, Natalia Martínez Lizaga, Miriam Seral Rodríguez, Ramón Launa Garcés, Ester Angulo Pueyo, Manolo Ridao López, Enrique Bernal Delgado','Gracia M Modroño Riaño', 'Eusebi Castaño Riera, María Zaforteza Dezcallar, María Vega Martín Martín','Flora Pérez Hernández, María Ángeles Lumbreras, Óscar Valcuende Mantilla', 'Cristina Colls, Èrica Martínez Solanas, Mireia Espallargues, Montserrat Vicente, Pere Carbonell, Thais de Pando, Ana de Andrés, Andrea Molina, Núria Escoda, Anna Berenguera, Anna Moleras, Daniel Garcia Abietar, Glòria Oliva, Carles Vilaplana, Manuel Medina, Neus Carrilero, Maria Pueyo', 'Jose Miguel Rodríguez Lugo, Sara Trujillo Alemán', 'Alejandra García Ortíz, Judith Ceruelo Bermejo, Nieves Martín Sobrino, Juan Carlos Solsona Perea', 'Isabel Hurtado, Julia Calabuig', 'José Manuel Izquierdo Palomares, Ángel Luis Mataix Sanjuan, María Isabel del Cura González, Tamara Alonso Safont, Javier Rubio Serrano, Daniel Cifo', 'Maite Almirantearena Legaz, Anabel Zaborras Aguado, Javier Gorricho Mendívil', 'Eduardo Millán Ortuondo, Ricardo Samper Ochotorena','Felix Rivera Sanz, Pilar Sáenz Ortiz, Martin Sebastian Mateo Berdini, Jose María Rodríguez Corral','Carlos Arenas Díaz, Víctor Rausell, José Eduardo Calle Urra, Amaya Jimeno Almazán, Rafael Herrero Delicado, Mariana Tobaruela Soto, María Onteniente Candela'),
