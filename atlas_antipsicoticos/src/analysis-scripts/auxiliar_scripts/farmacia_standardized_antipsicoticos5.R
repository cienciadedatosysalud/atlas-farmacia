...
select
	...
	zbs_residencia_cd,
	ccaa_cd,
	atc_farmaco_dispensado_cd[1:5] as atc_farmaco_dispensado,
...
for(variable_indicator in c('ddd_por_envase','pvp_nomenclator_nm')){
  ...
  for(año in lista_años){
    ...
    indicators_list <- c('N05AA','N05AB','N05AC','N05AD','N05AE','N05AF','N05AH','N05AG',
                         'N05AL','N05AN','N05AX')
  ...
  write.csv(indicators_rates_total, file = paste0(outputs_path,"antipsicoticos5_indicators_rates_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
  write.csv(estadisticosobs_total, file = paste0(outputs_path,"antipsicoticos5_estadisticosobs_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
}
