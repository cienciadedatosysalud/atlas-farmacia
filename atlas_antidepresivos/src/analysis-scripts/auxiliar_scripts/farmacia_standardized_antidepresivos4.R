...
select
	...
	zbs_residencia_cd,
	ccaa_cd,
	atc_farmaco_dispensado_cd[1:4] as atc_farmaco_dispensado,
...
for(variable_indicator in c('ddd_por_envase','pvp_nomenclator_nm')){
  ...
  write.csv(indicators_rates_total, file = paste0(outputs_path,"antipsicoticos4_indicators_rates_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
  write.csv(estadisticosobs_total, file = paste0(outputs_path,"antipsicoticos4_estadisticosobs_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
}
