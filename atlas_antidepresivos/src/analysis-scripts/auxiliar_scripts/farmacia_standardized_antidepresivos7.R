...
getPoblacionRef <- function(año){
  ...
  poblacion_ref <- dbGetQuery(conn = con, paste0("SELECT * FROM zona_basica_ine where sexo_cd IN (1,2) and año_cd = ",año))
  poblacion_ref <- poblacion_ref %>% dplyr::rename(pop=n_poblacion)
...
select
	...
	zbs_residencia_cd,
	ccaa_cd,
	atc_farmaco_dispensado_cd as atc_farmaco_dispensado,
...
select a.* from (select atc_farmaco_dispensado as atc_farmaco_dispensado ,ccaa_cd, zbs_residencia_cd ,sexo_cd, grupo_edad_cd , 
...
from partial_df where año = ",año," and sexo_cd in (1,2) group by atc_farmaco_dispensado ,ccaa_cd, zbs_residencia_cd ,sexo_cd, grupo_edad_cd) a"
  ))
...
getRatesByIndicator <- function(data,population_ref,population_aux){
  ...
  if(variable_indicator_ == 'ddd'){
    ...
    data_clean <- left_join(x=data_clean, y = population_ref_[c("grupo_edad_cd","sexo_cd","st_pop")])
...
getRates <- function(indicator,data,patient_variables,population,population_ref,variable_indicator){
  ...
  data<-left_join(x=population_aux,y=data,by=c("zbs_residencia_cd","grupo_edad_cd",
                                               "sexo_cd","codatzbs","n_zbs"))
...
writeResults2 <- function(indicators_rates, indicators_list,variable_indicator){
  ...
  for(i in 1:length(objeto)){
    ...
    #     # componente sistematico de variacion
    # 
    # csv<-(1/length(te))*(sum((casos-esp)^2/esp^2)-sum(1/esp))
    # csv95<-(1/length(te[te>quantile(te, 0.05)& te<quantile(te, 0.95)]))*( sum((casos[te>quantile(te, 0.05)& te<quantile(te, 0.95)]-esp[te>quantile(te, 0.05)& te<quantile(te, 0.95)])^2/esp[te>quantile(te, 0.05)& te<quantile(te, 0.95)]^2)-sum(1/esp[te>quantile(te, 0.05)& te<quantile(te, 0.95)]))
...
for(variable_indicator in c('ddd_por_envase','pvp_nomenclator_nm')){
  ...
  for(año in lista_años){
    ...
    indicators_list <- c("N06AA02","N06AA04","N06AA06","N06AA09","N06AA10","N06AA12",
                         "N06AA16","N06AA21","N06AB03","N06AB04","N06AB05","N06AB06",
                         "N06AB08","N06AB10","N06AF04","N06AG02","N06AX01","N06AX03",
                         "N06AX05","N06AX11","N06AX12","N06AX14","N06AX16","N06AX18",
                         "N06AX21","N06AX22","N06AX23","N06AX26")
  ...
  write.csv(indicators_rates_total, file = paste0(outputs_path,"antipsicoticos7_indicators_rates_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
  write.csv(estadisticosobs_total, file = paste0(outputs_path,"antipsicoticos7_estadisticosobs_",as.character(variable_indicator_),".csv"), 
            row.names = FALSE)
}
