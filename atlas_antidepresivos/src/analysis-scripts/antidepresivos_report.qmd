---
title: "ATLAS farmacia: Variaciones en el consumo y gasto de fármacos antidepresivos por zona básica de salud en el Sistema Nacional de Salud"
lang: es
author:
...
nomenclator_df <- read.csv(file = 'nomenclator_antidepresivos.csv',colClasses = c('codigo_nacional_farmaco_cd'='character'))
...
dbExecute(con, "CREATE OR REPLACE VIEW envase_dispensado_view_imao AS SELECT envase_id::VARCHAR || '-' || year(fecha_dispensacion_dt)::VARCHAR as envase_id, fecha_dispensacion_dt, paciente_id, edad_nm, sexo_cd, tsi_cd, zbs_residencia_cd, ccaa_cd, a.codigo_nacional_farmaco_cd,n.atc_farmaco_dispensado_cd
, n.ddd_nomenclator_nm, n.ddd_por_envase, n.pvp_nomenclator_nm, n.precio_por_envase_nm  FROM (SELECT * from main.envase_dispensado 
WHERE tsi_cd is NULL or tsi_cd NOT IN ('8')) a
 ...
 where n.codigo_nacional_farmaco_cd is not null AND n.atc_farmaco_dispensado_cd IN ('N06AF04','N06AG02')")
...
WHERE tsi_cd is NULL or tsi_cd NOT IN ('8')) a
 LEFT JOIN main.nomenclator n
 ON a.codigo_nacional_farmaco_cd = n.codigo_nacional_farmaco_cd
 where n.codigo_nacional_farmaco_cd is not null AND n.atc_farmaco_dispensado_cd NOT IN ('N06AF04','N06AG02')")
...
source('./auxiliar_scripts/farmacia_standardized_antidepresivos4.R')
rm(list=ls())
log_info('Calculando tasas estandarizadas e indicadores estadísticos para ATC 5 caracteres')
source('./auxiliar_scripts/farmacia_standardized_antidepresivos5.R')
rm(list=ls())
log_info('Calculando tasas estandarizadas e indicadores estadísticos para ATC 7 caracteres')
source('./auxiliar_scripts/farmacia_standardized_antidepresivos7.R')
rm(list=ls())
```
...
write.table(df, file = paste0(outputs_path,"antidepresivos_estadisticosobs.csv"),
            row.names = FALSE, sep = '|')
...
write.table(df, file = paste0(outputs_path,"antidepresivos_dhd_pvp_population_values.csv"),
            row.names = FALSE, sep = '|')
...
write.table(df_, file = paste0(outputs_path,"antidepresivos_dhd_pvp_resumen_anual.csv"),
            row.names = FALSE,  sep = '|')
...
from
	...
	from
		main.envase_dispensado_view
	WHERE
		(atc_farmaco_dispensado_short in ('N06AA','N06AB','N06AF','N06AG','N06AX')))
GROUP BY
	año_cd,
	atc_farmaco_dispensado_short
")
...
select
	...
	precio_por_envase_nm
from
	main.envase_dispensado_view_imao)
select
	a.*
from
	(
...
write.table(aggregated_output,'../../outputs/aggregated_outputs_imao.csv', sep = '|', row.names = FALSE)
rm(aggregated_output)

dbExecute(con,"DROP VIEW IF EXISTS envase_dispensado_imao")
...
La depresión es la enfermedad mental más prevalente en nuestro país con un importante impacto social, familiar y económico. El grupo de psicofármacos más dispensado en problemas de salud mental es el de los antidepresivos (AD). Su uso es mayor en mujeres que en hombres, en niveles más bajos de renta y con una dispensación mayor conforme menor es el tamaño del municipio [@ministerio_de_sanidad_subdireccion_general_de_informacion_sanitaria_salud_2021].

Los AD son la primera línea de tratamiento en depresión moderada o grave aunque también incluyen otras indicaciones como trastornos de ansiedad, dolor neuropático, insomnio y cefaleas y migrañas. El objetivo terapéutico de los AD es conseguir la remisión de síntomas depresivos y la prevención de recaídas a largo plazo, no recomendándose su utilización en pacientes con síntomas depresivos leves. Además, es aconsejable que la prescripción farmacológica de AD vaya acompañada de terapia psicológica.

Diversos estudios han puesto de manifiesto una escasa diferencia entre el efecto de los AD y el placebo e importantes efectos secundarios por el uso de los mismos [@moncrieff_efficacy_2005],[@fournier_antidepressant_2010],[@coupland_antidepressant_2011], con una clara recomendación de solo considerar su uso para personas con depresión moderada o grave y no utilizar AD en depresión leve puesto que la relación riesgo-beneficio es baja [@noauthor_nice_nodate].

Las dificultades de acceder a atención especializada han provocado que cada vez más, las personas acudan a Atención Primaria, en busca de soluciones para problemas sociales y vitales que habitualmente implican que se recete un medicamento con una utilidad dudosa en esos casos, lo que provoca una medicalización innecesaria [@marquez_medicalizacion_2023],[@orueta_sanchez_medicalizacion_nodate],[@varela_pinon_es_nodate]. El resultado final es que el consumo de AD ha experimentado un extraordinario crecimiento como tratamiento de muchos problemas consultados en atención primaria.

Durante la última década, la tendencia en el uso de fármacos AD en España, fundamentalmente el consumo de AD de segunda generación, ha aumentado. El uso de AD en 2023, medido a través de DHD (Dosis Diaria Definida/1000 habitante/día), ha aumentado en un 54 % desde 2010, de 64,7 DHD en 2010 a 99,3 DHD en 2023. Los Inhibidores selectivos de recaptación de serotonina (58%) y Otros AD (38,5%), representaron el 96,5 % del consumo de DHD en 2023 [@agencia_espanola_de_medicamentos_y_productos_sanitarios_utilizacion_2024].

Según datos de la OCDE [@noauthor_oecd_nodate] para 2022, España se encontraba entre los 4 países europeos donde hay una mayor prevalencia en el uso de estos medicamentos (98,8 DHD), solo superado por Islandia, Portugal y Suecia.
...
Dadas las dudas sobre la eficacia en algunas de las indicaciones de los fármacos AD, este proyecto de investigación pretende, esencialmente, aportar información empírica sobre la variabilidad en los patrones de consumo de medicamentos, siendo sus objetivos describir la variabilidad en las dosis dispensadas en el uso poblacional (DHD=número de DDD por 1000 habitantes y día) y en el gasto farmacéutico en la dispensación de fármacos de los AD dispensados a través de receta en oficina de farmacia, entre las zonas básicas de salud de las CCAA participantes.
...
Estudio observacional, retrospectivo, longitudinal y ecológico de las tasas estandarizadas de consumo y gasto farmacéutico en el grupo de fármacos antidepresivos (N06A), subgrupos terapéuticos y AD de primera y segunda generación (ver @sec-anexo). Estas tasas se estimaron como DHD (Dosis Diaria Definida (DDD) por cada 1000 habitantes y día) en cada zona básica de salud.
...
Se estudiarán los subgrupos de antidepresivos en función del ATC (5 dígitos) del fármaco que se corresponden con:

- Inhibidores no selectivos de recaptación de monoaminas (*N06AA*), 

- Inhibidores selectivos de recaptación de serotonina (*N06AB*), 

<!-- - Inhibidores de la monoaminooxidasa, no selectivos (*N06AF*),  -->

<!-- - Inhibidores de la monoaminooxidasa tipo A (*N06AG*),  -->

- Otros antidepresivos (*N06AX*).
...
## Distribución de la población con dispensaciones de antidepresivos por edad y sexo en el último año disponible 
::: {.justify}
...
## Porcentaje de población con dispensaciones de antidepresivos por edad y sexo en el último año disponible 
::: {.justify}
...
df_list_indicators <- data.frame(indicators = c('N06AA','N06AB','N06AX'),
                                 descr = c('Inhibidores no selectivos de recaptación de monoaminas', 
...
list_atc_4 <- data.frame(codigo = c('N06AA','N06AB','N06AX'),
                    descripcion = c('Inhibidores no selectivos de recaptación de monoaminas (N06AA)',
'Inhibidores selectivos de recaptación de serotonina (N06AB)',
'Otros antidepresivos (N06AX)'))
list_atc_7 <- data.frame(codigo = c('N06AA02',
'N06AA04',
'N06AA06',
'N06AA09',
'N06AA10',
...
'N06AX23',
'N06AX26'),
  descripcion = c('Imipramina (N06AA02)',
...
'Duloxetina (N06AX21)',
'Agomelatina (N06AX22)',
'Desvenlafaxina (N06AX23)',
'Vortioxetina (N06AX26)'))
...
plot_evo_temp_atc_nenv[[1]]$N06AA
```
...
plot_evo_temp_atc_nenv[[2]]$N06AB
```
...
plot_evo_temp_atc_nenv[[3]]$N06AX
```
...
::: {.justify}
Se muestra el consumo (DHD) para el grupo mayor N06A por ZBS en el último año disponible, ordenados de menor a mayor y coloreados por quintiles.
:::
::: {.callout-tip}
## Nota informativa
...
consumo_zbs_total_ <- read.csv('../../outputs/antidepresivos_dhd_pvp_population_values.csv', sep = '|')
consumo_zbs_total_ <- consumo_zbs_total_ %>% filter(year %in% max(year,na.rm=TRUE))
consumo_zbs_total_ <- consumo_zbs_total_ %>% filter(variable %in% 'dhd', indicator %in% 'N06A')
...
<!-- df_list_indicators <- data.frame(indicators = c('N06AA','N06AB','N06AF','N06AG','N06AX'), -->
<!--                                  descr = c('Inhibidores no selectivos de recaptación de monoaminas',  -->
<!--                                            'Inhibidores selectivos de recaptación de serotonina',  -->
<!--                                            'Inhibidores de la monoaminooxidasa, no selectivos', -->
...
::: {.justify}
Se muestra el porcentaje de envases dispensados de cada grupo de antidepresivos respecto al total de envases dispensados para cada año de estudio.
:::
...
plot <- ggplot(df_,
  ...
  scale_fill_manual(values = c('N06AA'="#66c2a5", 'N06AB'="#fc8d62", 'N06AX'='#a6d854')) +
...
data <- read.csv('../../outputs/antidepresivos_dhd_pvp_population_values.csv', sep = "|",colClasses=c("codatzbs"="character"))

data <- data %>% filter(indicator %nin% 'N06A')
...
```{r,message=FALSE,warning=FALSE, error=FALSE}
#| label: todos codigos
df_codigos <- data.frame(cod = c('N06AA02',
'N06AA04',
'N06AA06',
'N06AA09',
'N06AA10',
